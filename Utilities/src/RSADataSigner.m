	...
	for(int i = 0;i<len;i++){
		...
		if( ('a' <= c && c <= 'z')
		   ...
		   || ('0' <= c && c <= '9') ){
            ...
            NSString *ts = [[NSString alloc] initWithBytes:&c length:1 encoding:NSUTF8StringEncoding];           // NSString* ts = [[NSString alloc]initWithCString:(char *)&c   encoding:NSUTF8StringEncoding];
            ...
            //NSString* ts = [[NSString alloc]initWithCString:(char *)&ts1  encoding:NSUTF8StringEncoding];
            ...
            //ts = [[NSString alloc]initWithCString:(char *)&ts2  encoding:NSUTF8StringEncoding];
